...
## Objective
Implement canonical `HashRouter` routes and route-level shells for `/runs`, `/runs/:runId`, and `/definitions/:workflowType`, including run listing/filtering, run-selection navigation semantics, and definition metadata route rendering with history fidelity.
...
- [x] Implement `/definitions/:workflowType` route with definition metadata rendering.
...
- `/definitions/:workflowType` renders definition metadata view.
