...
## Objective
Close the staged web-spec delta by extending representative browser E2E coverage, widening ownership ledgers to the new behavior/test ranges, and updating CI gates so the new start-workflow, drill-down, transition-history, feedback, and logs semantics remain enforced.
...
- [ ] Add representative E2E happy-path coverage for start workflow and child-drill-down/transition-history observability flows.
...
- `apps/workflow-web/test/e2e/web-child-drilldown-history-happy-path.spec.ts`
...
- Representative browser coverage exists for start workflow and child drill-down / transition-history happy paths.
...
- `pnpm --filter @composable-workflow/workflow-web run test:e2e -- --grep "child drilldown"`
...
| WEB-18-GATE-002 | `.github/workflows/ci.yml` | CI runs exact targeted checks for the new start, graph, history, feedback, and logs integration specs. |
...
| WEB-18-GATE-004 | `apps/workflow-web/test/e2e/web-child-drilldown-history-happy-path.spec.ts` | Representative child-drill-down and transition-history browser happy-path coverage is present and gated. |
