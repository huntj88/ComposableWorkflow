...
## Objective
Implement the Transition History panel as an ordered execution narrative with nested child histories, persistent expand/collapse state across live updates, and deterministic coordination with the events timeline and link-filter time-range behavior.
...
- Selection synchronization between history and timeline uses shared UI state so panel updates stay deterministic across stream events.
...
- [x] Coordinate history-entry selection with events-timeline scrolling.
