...
- Graph contract drift validation is implemented as static integration tests that compare `workflow-api-types-spec.md` §5 and web spec Sections 6.6 and 8.5.
...
- Cross-spec graph lock is enforced as a three-way contract alignment gate across `workflow-api-types-spec.md` §5, web spec Sections 6.6/8.5, and `@composable-workflow/workflow-api-types` graph contract surfaces.
- Coverage ownership for `B-API-010`, `B-CONTRACT-007`, and `ITX-033` is assigned to this task.

## Interface/Schema Contracts
- Graph contract source: `workflow-api-types-spec.md` §5.
...
- [x] Add graph contract lock drift test for `workflow-api-types-spec.md` §5 vs web spec Sections 6.6 and 8.5.
- [x] Extend graph contract lock drift test to assert three-way alignment across `workflow-api-types-spec.md` §5, web spec Sections 6.6/8.5, and graph contract identifiers exported by `@composable-workflow/workflow-api-types`.
...
## Acceptance Criteria
- A drift test fails when `workflow-api-types-spec.md` §5 and web spec Sections 6.6/8.5 diverge on graph identity/overlay contract semantics.
- A drift test fails when graph identity/overlay semantics drift across any of the three lock artifacts: `workflow-api-types-spec.md` §5, web spec Sections 6.6/8.5, or `@composable-workflow/workflow-api-types` graph contract surfaces.
...
- Command: `pnpm --filter @composable-workflow/workflow-server exec vitest run test/integration/contract/graph-contract-lock-drift.spec.ts`
  - Expected: passes when `workflow-api-types-spec.md` §5, web graph-spec sections, and shared graph contract exports are aligned; fails on drift.
...
## Spec/Behavior Links
- Spec: `workflow-api-types-spec.md` §5.1, §5.2, §5.3; `docs/architecture.md` (criterion 21).
...
| B-CONTRACT-007 | `test/integration/contract/graph-contract-lock-drift.spec.ts` | `workflow-api-types-spec.md` §5 and web spec Sections 6.6/8.5 remain aligned on graph identity and overlay semantics; drift fails CI. |
