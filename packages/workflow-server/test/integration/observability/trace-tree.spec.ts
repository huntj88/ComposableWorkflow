...
const runRepository: RunRepository = {
  ...
  getRunSummary: async (_client, runId) => ({
    runId,
    workflowType: runId === 'wr_child' ? 'wf.child' : 'wf.parent',
    workflowVersion: '1.0.0',
    lifecycle: 'running',
    currentState: 'start',
    parentRunId: runId === 'wr_child' ? 'wr_parent' : null,
...
describe('observability trace tree', () => {
  it('builds parent/child span hierarchy for transitions, commands, and child runs', async () => {
    ...
    await repository.appendEvent({} as never, {
      ...
      runId: 'wr_child',
      eventType: 'workflow.started',
      timestamp: '2026-02-21T00:00:06.000Z',
    });
    await repository.appendEvent({} as never, {
      eventId: 'evt_8',
      runId: 'wr_child',
      eventType: 'workflow.completed',
      timestamp: '2026-02-21T00:00:07.000Z',
    });
    await repository.appendEvent({} as never, {
      eventId: 'evt_9',
      runId: 'wr_parent',
      eventType: 'workflow.completed',
      timestamp: '2026-02-21T00:00:08.000Z',
    });

    const spans = tracing.listRecordedSpans();
    const parentRoot = findSpan(spans, 'workflow.run', 'wr_parent');
    const transition = findSpan(spans, 'workflow.transition', 'wr_parent');
    const command = findSpan(spans, 'workflow.command', 'wr_parent');
    const childLaunch = findSpan(spans, 'workflow.child', 'wr_parent');
    const childRoot = findSpan(spans, 'workflow.run', 'wr_child');
